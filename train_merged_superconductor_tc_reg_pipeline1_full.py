...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Train the TC_reg_pipeline_1 best GNN regressor on the full merged dataset."
        )
    )
    parser.add_argument(
        "--csv",
    ...
    parser.add_argument(
        "--output-model",
        default="tc_reg_pipeline1_full_gnn_model.joblib",
    ...
    print("Starting TC_reg_pipeline_1 full-dataset training setup")
    ...
    print(f"Training on all valid rows: {len(df)}")
    print("Building full-dataset features...")
    X_full, y_full = build_features(df)
    ...
    model = train_model(X_full, y_full, model=existing_model, epochs=args.epochs)

    os.makedirs(os.path.dirname(output_model) or ".", exist_ok=True)
    print(f"Saving model to: {output_model}")
    joblib.dump(model, output_model)
    print("Full-dataset regression training setup is complete")
