...
def main():
    ...
    # --- Log-target scaling: train on log1p(Tc) to handle the skewed Tc range ---
    # (Metrics and saved predictions are inverted back to the real Tc scale below,
    #  so R2/MAE/RMSE remain honest, real-Kelvin numbers.)
    y_train_arr = np.asarray(y_train, dtype=float)
    if np.any(y_train_arr < 0):
        raise ValueError("Negative Tc encountered; cannot apply log scaling.")
    y_train_log = np.log1p(y_train_arr)
    print("Applying log1p scaling to training targets (Tc -> log1p(Tc))")
    ...
    model = train_model(X_train, y_train_log, model=existing_model, epochs=args.epochs)
    ...
    if not test_df.empty:
        ...
        if X_test:
            ...
            y_pred_log = predict(model, X_test)
            # invert log1p back to real Tc scale for honest evaluation
            y_pred = np.expm1(np.asarray(y_pred_log, dtype=float))
            y_pred = np.clip(y_pred, 0, None)  # Tc cannot be negative
